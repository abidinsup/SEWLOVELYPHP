...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($result && $stmt->rowCount() > 0) {
            ...
            // Fetch mitra data for notification & WhatsApp
            $stmt2 = $pdo->prepare("
                SELECT p.whatsapp_number, p.full_name, p.bank_name, p.account_number, p.user_id, t.amount
                FROM transactions t
            ...
            if ($mitra) {
                ...
                $amountFormatted = 'Rp ' . number_format((float)$mitra['amount'], 0, ',', '.');
                ...
                if ($action === 'approve') {
                    sendNotificationToUser(
                        ...
                        $mitra['user_id'],
                        'Penarikan Disetujui! ✅',
                        'Penarikan ' . $amountFormatted . ' ke ' . $mitra['bank_name'] . ' telah diproses',
                        'withdrawal',
                        ['status' => 'approved', 'amount' => (string)$mitra['amount']]
                    );
                    ...
                    sendNotificationToUser(
                        ...
                        $mitra['user_id'],
                        'Penarikan Ditolak ❌',
                        'Penarikan ' . $amountFormatted . ' tidak disetujui. Hubungi admin untuk info lebih lanjut.',
                        'withdrawal',
                        ['status' => 'rejected', 'amount' => (string)$mitra['amount']]
                    );
                }
            }
